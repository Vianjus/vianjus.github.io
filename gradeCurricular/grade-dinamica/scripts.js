...
const getRequires = (course) => {
    return course.requires.map(id => {
      if(isNaN(id)){
        const requiredCourse = courses.find(c => c.id === id);
        if(!requiredCourse) {
          console.error(`Disciplina não encontrada: ${id} (requerida por ${course.id})`);
          return null; // ou tratar como bloqueado
        }
        return requiredCourse;
      }
      return id;
    }).filter(r => r !== null); // Remove requisitos inválidos
  };
